...
public class VideoEditor extends JPanel {
	 ...
	 */
	private VideoEditor() {
		setLayout(new MigLayout("", "[400px,grow][400px,grow]",
				"[400px,grow][250px,grow]"));
		...
		preview.setBorder(new TitledBorder(new EtchedBorder(
				EtchedBorder.LOWERED, null, null), "Text Preview",
				TitledBorder.LEADING, TitledBorder.TOP, null, null));
		...
		textPanel.setBorder(new TitledBorder(new BevelBorder(BevelBorder.LOWERED, null, null, null, null), "Text Editing", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		add(textPanel, "cell 0 1 2 1,grow");
		textPanel.setLayout(new MigLayout("",
				"[200px,grow][200px,grow][200px,grow][200px,grow]",
		...
		((JSpinner.DefaultEditor) _fontSpinner.getEditor()).getTextField()
		.setEditable(false);
		;
		...
		((JSpinner.DefaultEditor) _minSpinner.getEditor()).getTextField()
		.setEditable(false);
		;
		...
		((JSpinner.DefaultEditor) _secSpinner.getEditor()).getTextField()
		.setEditable(false);
		;
	...
	public void setUpListener() {
		...
		_saveButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				for (Enumeration<AbstractButton> buttons = _group.getElements(); buttons
						.hasMoreElements();) {
					AbstractButton button = buttons.nextElement();
					if (button.isSelected()
							&& !button.getText().equals("Title")) {
						isTitle = false;
						break;
					}
				}
				if (((Integer) _minSpinner.getValue()) == 0
						&& ((Integer) _secSpinner.getValue()) == 0) {
		...
		_fontSpinner.addChangeListener(new ChangeListener() {
			...
			public void stateChanged(ChangeEvent e) {
				float newSize = ((Integer) ((JSpinner) e.getSource())
						.getValue()).floatValue();
		...
		_colourButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				Color newCol = JColorChooser.showDialog(
						VideoEditor.getInstance(), "Choose font colour",
						_textArea.getForeground());
				if (newCol != null){
					_textArea.setForeground(newCol);
					writeSetting(null, null, String.format("%02x%02x%02x%02x",
							newCol.getRed(), newCol.getGreen(), newCol.getBlue(),
							newCol.getAlpha()), null, null);
				}
			}
		});
	 ...
	 * @param text
	 */
	private void writeSetting(String font, Integer size, String color,
	...
	public void setUpTextArea() {
		...
		String output = Library.outputDir + File.separator
				+ filenameNoExtension + ".cfg";
		...
		_textArea.setFont(new Font(settings[0], Font.PLAIN, Integer
				.parseInt(settings[1]))); // Might have to check for invalid
		// fonts
