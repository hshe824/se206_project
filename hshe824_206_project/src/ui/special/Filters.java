...
public class Filters extends Pane {
	...
	private Filters() {
		...
		textPanel.setBorder(new TitledBorder(new BevelBorder(BevelBorder.LOWERED, null, null, null, null),
				"Filters to apply", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		...
		_colourButton.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				Color newCol = JColorChooser.showDialog(Filters.getInstance(), "Choose font colour", Color.red);
				if (newCol != null) {
					_colourTint = newCol;
				}
			}
				...
				filterTask.addPropertyChangeListener(new PropertyChangeListener() {
					...
					public void propertyChange(PropertyChangeEvent evt) {
						if ("failure".equals(evt.getPropertyName())) {
							...
							JOptionPane.showMessageDialog(null, "Adding of filter to:" + _inputFile
									+ " was successful!", "Filter added!", JOptionPane.INFORMATION_MESSAGE);
	...
	public void setInputFile(String _inputFile) {
		...
		_outputLocationFiltered = Library.outputDir + File.separator + filenameNoExtension + "[FILTER_ADDED-VAMIX].mp4";
